...
def train_model(selected_features, perform_tuning, n_estimators, max_depth, min_sample):
    data = pd.read_csv(os.path.join(current, 'HumanData', 'output.csv'))
    ...
    if perform_tuning:
        ...
        param_grid = {
            ...
            'model__estimator__min_samples_split': min_sample
        }
...
def index():
    if request.method == 'POST':
        ...
        pipeline_rf, pipeline_gb, X_test_cleaned, y_test_cleaned, y_pred_rf, y_pred_gb = train_model(selected_features, perform_tuning, n_estimators, max_depth, min_samples_split)
